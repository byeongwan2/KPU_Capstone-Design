Shader "PolygonR/PBR_Character_Overlay" {
	Properties {
		...
		_SkinTone("Skin Tones Mix  (RGB)", 2D) = "grey" {}
		_SkinMux("Skin Tone Mix", Float) = 1.0
	...
	SubShader {
		...
		sampler2D _MainTex, _MetallicGlossMap,  _EmissionMap, _BumpMap, _Rough, _SkinTone; //_OcclusionMap
		...
		half _Glossiness, _Metallic, _EmissionFactor, _BumpScale, _OcclusionStrength, _RoughLerp, _DamageFX, _SkinMux;
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			fixed4 skinTone = tex2D(_SkinTone, RescaledUV);

			half gray2 = (c2.r + c2.g + c2.b) / 3;
			
			c2 = lerp(c2, lerp(1 - (2 * (1 - c2)) * (1 - skinTone), (2 * c2) * skinTone, step(gray2, 0.5f)) * (1 + _SkinMux), ( colorMask.r + colorMask.g + colorMask.b) * _SkinMux);
