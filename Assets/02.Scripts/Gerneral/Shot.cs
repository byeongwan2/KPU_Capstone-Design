...
public class Shot : Behaviour
{
    ...
    public override void Work<T>(TYPE _type)        //T는 AttackObject
    { 
        var bullet = PrefabSystem.instance.ActivePrefab(_type).GetComponent<T>();         //게임오브젝트가 리턴되므로 이부분 수정해야함 // SetActive(true) 상태로 리턴
